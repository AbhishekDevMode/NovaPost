...
const PostDetail = () => {
  ...
  useEffect(() => {
    const fetchPostAndComments = async () => {
      try {
        ...
        const commentsRes = await axios.get(`http://localhost:5000/api/posts/${data._id}/comments`);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`http://localhost:5000/api/posts/${post._id}/like`, {}, config);
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const { data } = await axios.post(`http://localhost:5000/api/posts/${post._id}/comments`, { text: newComment }, config);
  ...
  if (!post) {
    return <div className="text-center p-12 text-gray-500">Post not found.</div>;
  }
      ...
      <div className="p-8 md:p-12">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
            ...
            <span className="flex items-center gap-1"><Clock size={16} /> {format(new Date(post.createdAt), 'MMMM d, yyyy')}</span>
          </div>
          ...
          <div className="flex items-center justify-between py-6 border-y border-gray-100">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleLike} className={`flex items-center gap-2 px-4 py-2 rounded-full border transition ${hasLiked ? 'bg-red-50 border-red-200 text-red-500' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                <Heart size={20} fill={hasLiked ? "currentColor" : "none"} className={hasLiked ? "text-red-500" : ""} />
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="mt-12 flex flex-wrap gap-2">
            {post.tags.map((tag, idx) => (
              <span key={idx} className="bg-gray-100 text-gray-600 px-3 py-1 rounded-md text-sm font-medium">
                #{tag}
              </span>
            ))}
          </div>
        ...
        <div className="mt-16 pt-12 border-t border-gray-100">
          ...
          {user ? (
            <form onSubmit={handleCommentSubmit} className="mb-10">
              ...
              <div className="flex justify-end mt-2">
                <button type="submit" className="bg-gray-900 hover:bg-black text-white px-6 py-2 rounded-lg font-medium transition shadow-md">
                  Post Comment
                </button>
              </div>
            </form>
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 mb-10 text-center">
              ...
              <Link to="/login" className="bg-indigo-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-indigo-700 transition inline-block shadow-md">
                Log in to Comment
              </Link>
            </div>
          )}
          ...
          <div className="space-y-6">
            {comments.map((comment) => (
              <div key={comment._id} className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
