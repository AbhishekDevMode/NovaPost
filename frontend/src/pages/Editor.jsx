...
const Editor = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchPost = async () => {
        try {
          const { data } = await axios.get(`http://localhost:5000/api/posts/id/${id}`); // wait, we don't have a route by id for GET, we use slug. Let's adjust backend or we can get it from dashboard analytics data if we had global state.
          // Since we need to get post by id for editing, let's fetch all author posts from analytics and find it.
          const config = { headers: { Authorization: `Bearer ${user.token}` } };
          const analyticsRes = await axios.get('http://localhost:5000/api/analytics', config);
  ...
  const handleCreateCategory = async (e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const { data } = await axios.post('http://localhost:5000/api/categories', { name: newCatName }, config);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await axios.put(`http://localhost:5000/api/posts/id/${id}`, postData, config);
      } else {
    ...
    <div className="max-w-5xl mx-auto bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-2/3 space-y-6">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Subtitle / Summary</label>
            <input
              type="text"
        ...
        <div className="lg:w-1/3 space-y-6">
          <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 space-y-6">
            <h3 className="font-bold text-gray-800 text-lg border-b border-gray-200 pb-2">Publish Settings</h3>
            ...
            <div>
              ...
              <form onSubmit={handleCreateCategory} className="flex gap-2">
                ...
                />
                <button type="submit" className="bg-gray-200 hover:bg-gray-300 text-gray-700 px-3 py-1.5 rounded-lg text-sm font-medium transition">
                  Add
                </button>
              </form>
            </div>
